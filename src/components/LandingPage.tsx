...
const TESTIMONIALS = [
  {
    ...
    quote: "Réservé en 5 minutes, le professionnel est arrivé à l'heure chez moi. Une prestation impeccable sans bouger de chez moi.",
    avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=200&auto=format&fit=crop',
  },
  {
    name: 'Sophia El K.',
    role: 'Cliente · Rabat',
    quote: "Enfin une appli qui rend la prise de rendez-vous simple. Les avis certifiés m'ont aidée à choisir le bon professionnel.",
    ...
    <div className="relative z-10 transition-colors duration-300">
        ...
        <div className="relative flex-1 flex flex-col justify-center overflow-hidden">
          ...
          <div className="relative z-10 px-6 md:px-16 py-14 md:py-20 font-open-sans">
            ...
            >
              ...
              <CategoryRail selected={null} onSelect={(id) => id ? onSelectCategory(id) : onFindNearby()} theme="dark" size="lg" hideAll />
            </motion.div>
            ...
            <div className="max-w-2xl">
              {profile && (profile.role === 'client' || profile.role === 'barber') ? (
                ...
                <>
                  ...
                  >
                    <button onClick={onFindNearby} className="btn-primary flex items-center gap-2 md:gap-3 !px-5 !py-3 md:!px-8 md:!py-3 text-[11px] md:text-sm !bg-gold/80 backdrop-blur-md border border-gold/40 hover:!bg-gold-light/90 shadow-lg shadow-gold/10 w-fit">
                      <MapPin size={16} className="shrink-0" />
                      Trouver un professionnel autour de moi
                    </button>
                    <button
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    transition={{ delay: 0.5 }}
                    className="flex flex-wrap gap-x-6 gap-y-4 sm:gap-x-8 mt-8 md:mt-16"
                  >
                    {[
                      { val: totalPros.toLocaleString('fr-FR'), label: 'Professionnels' },
                      { val: totalUsers.toLocaleString('fr-FR'), label: 'utilisateurs' },
                      { val: '4.9/5', label: 'Score' },
                      { val: dayVisitors.toLocaleString('fr-FR'), label: "Visiteurs aujourd'hui" },
                      { val: monthVisitors.toLocaleString('fr-FR'), label: 'Visiteurs ce mois-ci' },
                    ].map((stat, i) => (
                      <div key={stat.label} className={`text-center md:text-left ${i > 0 ? 'pl-6 sm:pl-8 border-l border-white/15' : ''}`}>
                        <div className="text-xl md:text-3xl font-bebas text-gold mb-1">{stat.val}</div>
                        <div className="text-[9px] md:text-[10px] text-white/70 uppercase tracking-widest font-bold whitespace-nowrap">{stat.label}</div>
                      </div>
                    ))}
                  </motion.div>
                </>
      ...
      <section id="services" className={`py-24 px-6 md:px-16 relative ${theme === 'dark' ? 'bg-dark-brown/40' : 'bg-cream'}`}>
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-warm-gray max-w-2xl mx-auto">Choisissez une prestation pour voir les professionnels qui la proposent près de chez vous.</p>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <div className="space-y-6">
              {[
                { icon: <MapPin size={18} />, title: 'Géolocalisation instantanée', desc: "Trouvez les professionnels disponibles autour de vous en temps réel." },
                { icon: <ShieldCheck size={18} />, title: 'Professionnels vérifiés (KYC)', desc: "CIN et selfie de validation contrôlés avant toute mise en relation." },
      ...
      <section id="how" className="py-24 px-6 md:px-16 border-y border-gold/10 relative overflow-hidden">
        ...
        >
          ...
          <h2 className={`text-5xl md:text-7xl mb-6 uppercase ${theme === 'dark' ? 'text-white' : 'text-black'}`}>Comment <span className="italic gold-gradient-text text-bebas underline decoration-gold/30">trouver votre professionnel</span></h2>
        ...
        <div className={`max-w-7xl mx-auto grid md:grid-cols-4 gap-0 border border-gold/10 relative z-10 ${theme === 'dark' ? 'bg-black/20' : 'bg-white'}`}>
          {[
            { num: '01', icon: <MapPin />, title: "Localisation", desc: "Activez votre position pour voir les professionnels autour de vous, où que vous soyez." },
            { num: '02', icon: <Sparkles />, title: "Exploration", desc: "Consultez les portfolios et avis certifiés des professionnels locaux." },
         ...
         <div className="grid md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
               ...
               <ul className="flex flex-col gap-3">
                 ...
                 <li><button onClick={onFindNearby} className="text-warm-gray text-sm hover:text-white transition-colors text-left">Trouver un professionnel</button></li>
               </ul>
            </div>
         </div>
